...
|6  |As an owner,<br /> I want to update a customer’s order<br /> So that I can modify info related to the order<br />|**`PUT /api/orders/:order_id` &ensp;: Edit a order**<br/>Payload:<br/><code>{<br/>&ensp;status(string between("NEW", "PAID", "CANCELED"))<br/>}</code><br/>Example:<br/><code>{<br/>&ensp;status: "NEW"<br/>}</code><br/><br/>You can consume this API with UI at URI `/orders` from the browser<br/><br/>For the status update from "NEW" INTO "CANCELED" at 5 P.M should be already run with command `bundle exec whenever --update-crontab --set environment=development`|
